    ...
    def _launch_browser(self, p, headless=True):
        if headless:
            try:
                ...
                self.log(f"Chromium launch failed: {first_error}")
                self.log("Attempting Playwright browser installation...")
    
                try:
                    driver_exe, driver_env = compute_driver_executable()
    
                    # Some Playwright versions return a string here;
                    # only mappings can be merged into os.environ.
                    ...
                    raise RuntimeError(
                        ...
                        f"Installation error: {second_error}"
                    )
